...
def main(debug = False, debug_detailed = False):
    ...
    appearsInTable = "CREATE TABLE IF NOT EXISTS appears_in (cname VARCHAR(64) NOT NULL, gtitle VARCHAR(64) NOT NULL, PRIMARY KEY (cname, gtitle), FOREIGN KEY (cname) REFERENCES game_character(name) ON UPDATE CASCADE ON DELETE CASCADE, FOREIGN KEY (gtitle) REFERENCES game(title) ON UPDATE CASCADE ON DELETE CASCADE);"
